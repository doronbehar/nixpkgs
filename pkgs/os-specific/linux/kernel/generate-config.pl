...
#
# While reading, also rewrite any option that was requested as an empty
# string: `make config` treats an empty interactive answer as "use the
# default", so a non-empty defconfig value (e.g. CONFIG_LOCALVERSION="-v8")
# must be replaced directly in the file. Options missing from .config stay
# absent, so the validation below still reports them as unused.
my %config;
{
