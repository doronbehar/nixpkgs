...
in rec {
  ...
  patches = []
    ++ stdenv.lib.optionals (!hasFeature "python-support" features) [
      # Without this, swig files of gnuradio-runtime are not added, and
      # gr-osmosdr build fails. Can't be upstreamed since they have moved away
      # from swig sometime after 3.8.2.0, see:
      # https://github.com/gnuradio/gnuradio/commit/9f6086161
      ./install-swig-even-without-python.patch
    ]
  ;
